...
start_update(){
  if [ -z $ss_basic_rule_update_url ]; then
    echo_date "没有设置更新规则url，使用默认地址:https://raw.githubusercontent.com/qizidev/fancyss-rule-updater/master/rules"
    ss_basic_rule_update_url="https://raw.githubusercontent.com/qizidev/fancyss-rule-updater/master/rules"
  fi
	# version dectet
	...
	version_chnroute61=$(cat /koolshare/ss/rules/version | sed -n 10p | sed 's/ /\n/g' | sed -n 1p)
	version_cdn1=$(cat /koolshare/ss/rules/version | sed -n 4p | sed 's/ /\n/g'| sed -n 1p)

	echo ==================================================================================================
	echo_date 开始更新shadowsocks规则，请等待...
	wget --no-check-certificate --timeout=8 -qO - "$ss_basic_rule_update_url"/version1 > /tmp/ss_version
	if [ "$?" == "0" ]; then
	...
	if [ "$ss_basic_gfwlist_update" == "1" ];then
		...
		if [ ! -z "$version_gfwlist2" ];then
			if [ "$version_gfwlist1" != "$version_gfwlist2" ];then
				...
				wget --no-check-certificate --timeout=8 -qO - "$ss_basic_rule_update_url"/gfwlist.conf > /tmp/gfwlist.conf
	...
	if [ "$ss_basic_chnroute_update" == "1" ];then
		...
		if [ ! -z "$version_chnroute2" ];then
			if [ "$version_chnroute1" != "$version_chnroute2" ];then
				...
				wget --no-check-certificate --timeout=8 -qO - "$ss_basic_rule_update_url"/chnroute.txt > /tmp/chnroute.txt
  ...
  if [ "$ss_basic_chnroute_update" == "1" ]; then
    echo_date " --------------------------------------------------------------------"
    if [ ! -z "$version_chnroute62" ]; then
      if [ "$version_chnroute61" != "$version_chnroute62" ]; then
        echo_date "检测到新版本chnroute6，开始更新..."
        echo_date "下载chnroute6到临时文件..."
        wget --no-check-certificate --timeout=8 -qO - "$ss_basic_rule_update_url"/chnroute6.txt >/tmp/chnroute6.txt
        md5sum_chnroute61=$(md5sum /tmp/chnroute6.txt | sed 's/ /\n/g' | sed -n 1p)
        if [ "$md5sum_chnroute61"x = "$md5sum_chnroute62"x ]; then
          echo_date "下载完成，校验通过，将临时文件覆盖到原始chnroute6文件"
          mv /tmp/chnroute6.txt /koolshare/ss/rules/chnroute6.txt
          sed -i "10s/.*/$git_line10/" /koolshare/ss/rules/version
          reboot="1"
          echo_date "【更新成功】你的chnroute6已经更新到最新！"
        else
          echo_date "下载完成，但是校验没有通过！"
        fi
      else
        echo_date "检测到chnroute6本地版本号和在线版本号相同，不进行更新!"
      fi
    else
      echo_date "chnroute6文件下载失败！"
    fi
  else
    echo_date "然而你并没有勾选chnroute更新！"
  fi
	...
	if [ "$ss_basic_cdn_update" == "1" ];then
		...
		if [ ! -z "$version_cdn2" ];then
			if [ "$version_cdn1" != "$version_cdn2" ];then
				...
				wget --no-check-certificate --timeout=8 -qO - "$ss_basic_rule_update_url"/cdn.txt > /tmp/cdn.txt
